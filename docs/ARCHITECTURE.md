...
flowchart TB
    ...
    subgraph Crew["The Last Mile Crew"]
      LEAD["Crew Lead"] --> SENTRY["Sentry"]
      SENTRY --> SCOUT["Scout"]
      SCOUT --> ROUTER["Router"]
    ...
    LEAD --> BEDROCK["Strands Agents SDK · Amazon Bedrock"]
...
- **Brief the Crew** runs the coordinated Sentry → Scout → Router → Dispatch fast path.
...
Nothing produced by the fast path is a separate black-box answer. The detailed pages surface the same structured outputs so an operator can inspect or modify them.
...
The web-facing boundary remains ECS/FastAPI. Bedrock and Strands support agent reasoning and explanations. EventBridge/AgentCore components support scheduled or hosted agent execution where configured. Documentation must label a component as planned if it is not active in the target deployment.
